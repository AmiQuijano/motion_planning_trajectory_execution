...
class CameraRecorder(Node):
    ...
    def wait_for_tf_and_frame(self):
        ...
        while rclpy.ok():
            ...
            try:
                ...
                if ("depth" in data and data["depth"] is not None and
    ...
    def run(self):
        ...
        while i < NUM_FRAMES and rclpy.ok():
            ...
            self.get_logger().info(f"Recorded frame {i+1}/{NUM_FRAMES}")

            # 4. Visualize:
            key = self.visualize_depth_color(color, depth)
            if key & 0xFF == ord("q") or key == 27:  # q or ESC close stop recording
                self.get_logger().info("Recording interrupted by user.")
                break

            i += 1 # Increment only if frame was successfully recorded

        # 5. Save all frames data to disk
